...
async function renderHome() {
    ...
    grid.innerHTML = POSTS.map((post) => postCardHtml(post, post.initialLikes || 0, 0)).join("");

    const { commentCounts, likeCounts } = await fetchAllCounts();

    grid.innerHTML = POSTS.map((post) => {
        const likeCount = (post.initialLikes || 0) + (likeCounts[post.id] || 0);
...
async function renderArticle() {
    ...
    const articleContent = await loadArticleContent(post.content);
    ...
    <hr class="divider" />
    <p class="article-content">${articleContent ? escapeHtml(articleContent).replace(/\n/g, '<br/>') : ''}</p>
